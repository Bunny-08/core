...
import { Clock, CheckCircle2, MoreVertical, Circle } from 'lucide-react';
    ...
    <div className="max-w-6xl mx-auto">
      <header className="mb-8 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400">You have {stats.inProgress} tasks in progress today.</p>
        </div>
        <div className="text-right">
          <p className="text-xs font-bold text-orange-600 uppercase tracking-widest">{today}</p>
        </div>
      </header>
          ...
          <section className="bg-white rounded-2xl border border-[#E5E7EB] dark:border-gray-800 shadow-sm overflow-hidden">
            ...
            <div className="divide-y divide-[#E5E7EB] dark:divide-gray-800">
              {[...filteredTasks]
                ...
                .slice(0, 5)
                .map((task) => (
                <div key={task.id} className="p-4 hover:bg-white dark:hover:bg-gray-800 flex items-center justify-between transition-colors group">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400">Due: {task.dueDate} • {task.priority}</p>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="lg:col-span-1">
          <section className="bg-white rounded-2xl border border-[#E5E7EB] dark:border-gray-800 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-[#E5E7EB] dark:border-gray-800">
              <h3 className="font-bold text-lg text-gray-900">Upcoming Festivals</h3>
            </div>
              ...
              {festivals.map((fest, idx) => (
                <div key={idx} className="flex items-start gap-4">
                  <div className="w-12 h-12 bg-orange-50 dark:bg-orange-950/30 rounded-xl flex flex-col items-center justify-center text-orange-600 shrink-0">
                    <span className="text-[10px] font-bold uppercase">{fest.date.split(' ')[0].substring(0, 3)}</span>
                    <span className="text-lg font-bold leading-none">{fest.date.split(' ')[1].replace(',', '')}</span>
                  </div>
                  <div>
                    <p className="font-semibold text-sm text-gray-900">{fest.name}</p>
                    <p className="text-xs text-gray-500">{fest.type} Holiday</p>
                  </div>
                </div>
              ))}
            </div>
